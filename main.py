...
def run_eval(agent, opt, datatype, max_exs=-1, write_log=False, valid_world=None):
    ...
    for _ in valid_world:
        ...
        cnt += opt['batchsize']
        if valid_world.epoch_done() or 0 < max_exs < cnt:
...
def main():
    ...
    train.add_argument('-et', '--evaltask',
                       help='task to use for valid/test (defaults to the one used for training if not set)')
    ...
    train.add_argument('-vtim', '--validation-every-n-secs', type=float, default=-1)
    train.add_argument('-vme', '--validation-max-exs', type=int, default=-1,
                       help='max examples to use during validation (default -1 uses all)')
    train.add_argument('-vp', '--validation-patience', type=int, default=5,
                       help=('number of iterations of validation where result '
                             + 'does not improve before we stop training'))
    train.add_argument('-dbf', '--dict-build-first', type='bool', default=True,
    ...
    while True:
        ...
        if 0 < opt['max_train_time'] < train_time.time():
            ...
            break
        if 0 < opt['log_every_n_secs'] < log_time.time():
